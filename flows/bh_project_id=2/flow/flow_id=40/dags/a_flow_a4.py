...
common_task = CommonTask(dag_id='a_flow_a4', dag_params={})
lineage_task = LineageTask(dag_id='a_flow_a4', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_82948eeef(**context):
    ...
    create_compute_82948eeef = PythonOperator(
        task_id='create_compute_82948eeef',
        python_callable=create_databricks_cluster_create_compute_82948eeef,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_82948eeef",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_5fe4b4df2_{{ ts_nodash }}",
    ...
    submit_job_5fe4b4df2 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_5fe4b4df2',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_82948eeef", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_82948eeef or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_f5e410a86 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_f5e410a86',
    ...
    start_flow_task >> create_compute_82948eeef
    create_compute_82948eeef >> submit_job_5fe4b4df2
    submit_job_5fe4b4df2 >> delete_compute_f5e410a86
    create_compute_82948eeef >> delete_compute_f5e410a86
    delete_compute_f5e410a86 >> end_flow_task
