...
common_task = CommonTask(dag_id='a_flow_12', dag_params={})
lineage_task = LineageTask(dag_id='a_flow_12', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_f86460e52(**context):
    ...
    create_compute_f86460e52 = PythonOperator(
        task_id='create_compute_f86460e52',
        python_callable=create_databricks_cluster_create_compute_f86460e52,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_f86460e52",
        "job_config": {
    ...
    submit_job_24241c166 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_24241c166',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_f86460e52", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_f86460e52 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_0ec4ce712 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_0ec4ce712',
    ...
    start_flow_task >> create_compute_f86460e52
    create_compute_f86460e52 >> submit_job_24241c166
    submit_job_24241c166 >> delete_compute_0ec4ce712
    create_compute_f86460e52 >> delete_compute_0ec4ce712
    delete_compute_0ec4ce712 >> end_flow_task
