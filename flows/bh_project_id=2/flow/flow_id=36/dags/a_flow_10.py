...
common_task = CommonTask(dag_id='a_flow_10', dag_params={})
lineage_task = LineageTask(dag_id='a_flow_10', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_81a4984fa(**context):
    ...
    create_compute_81a4984fa = PythonOperator(
        task_id='create_compute_81a4984fa',
        python_callable=create_databricks_cluster_create_compute_81a4984fa,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_81a4984fa",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_0f0476e61_{{ ts_nodash }}",
    ...
    submit_job_0f0476e61 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_0f0476e61',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_81a4984fa", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_81a4984fa or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_cca446d4c = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_cca446d4c',
    ...
    start_flow_task >> create_compute_81a4984fa
    create_compute_81a4984fa >> submit_job_0f0476e61
    submit_job_0f0476e61 >> delete_compute_cca446d4c
    create_compute_81a4984fa >> delete_compute_cca446d4c
    delete_compute_cca446d4c >> end_flow_task
