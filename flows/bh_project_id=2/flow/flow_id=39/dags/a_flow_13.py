...
common_task = CommonTask(dag_id='a_flow_13', dag_params={})
lineage_task = LineageTask(dag_id='a_flow_13', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_55648f1c5(**context):
    ...
    create_compute_55648f1c5 = PythonOperator(
        task_id='create_compute_55648f1c5',
        python_callable=create_databricks_cluster_create_compute_55648f1c5,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_55648f1c5",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_99744b090_{{ ts_nodash }}",
    ...
    submit_job_99744b090 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_99744b090',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_55648f1c5", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_55648f1c5 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_45c491cad = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_45c491cad',
    ...
    start_flow_task >> create_compute_55648f1c5
    create_compute_55648f1c5 >> submit_job_99744b090
    submit_job_99744b090 >> delete_compute_45c491cad
    create_compute_55648f1c5 >> delete_compute_45c491cad
    delete_compute_45c491cad >> end_flow_task
