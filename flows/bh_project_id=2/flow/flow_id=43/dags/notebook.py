...
common_task = CommonTask(dag_id='notebook', dag_params={})
lineage_task = LineageTask(dag_id='notebook', dag_params={})
...
with DAG(
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_2a34958a6",
        "job_config": {
            "job_type": "notebook",
    ...
    submit_notebook_33c4fc8b2 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_33c4fc8b2',
    ...
    def create_databricks_cluster_create_compute_2a34958a6(**context):
        from airflow_plugins.cloud_factory import CloudFactory
    ...
    create_compute_2a34958a6 = PythonOperator(
        task_id='create_compute_2a34958a6',
        python_callable=create_databricks_cluster_create_compute_2a34958a6,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_2a34958a6", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_2a34958a6 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_e204acdfe = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_e204acdfe',
    ...
    create_compute_2a34958a6 >> submit_notebook_33c4fc8b2
    start_flow_task >> create_compute_2a34958a6
    submit_notebook_33c4fc8b2 >> delete_compute_e204acdfe
    create_compute_2a34958a6 >> delete_compute_e204acdfe
    delete_compute_e204acdfe >> end_flow_task
