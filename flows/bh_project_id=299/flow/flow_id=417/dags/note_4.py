...
common_task = CommonTask(dag_id='note_4', dag_params={})
lineage_task = LineageTask(dag_id='note_4', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_b6742b9fe(**context):
        ...
        payload = (
            {
                "cluster_name": "dbricks_test_cluster_01",
    ...
    create_compute_b6742b9fe = PythonOperator(
        task_id='create_compute_b6742b9fe',
        python_callable=create_databricks_cluster_create_compute_b6742b9fe,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_b6742b9fe",
        "job_config": {
            "job_type": "notebook",
    ...
    submit_notebook_ec34ec36a = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_ec34ec36a',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_b6742b9fe", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_b6742b9fe or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_ca74d707a = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_ca74d707a',
    ...
    start_flow_task >> create_compute_b6742b9fe
    create_compute_b6742b9fe >> submit_notebook_ec34ec36a
    submit_notebook_ec34ec36a >> delete_compute_ca74d707a
    create_compute_b6742b9fe >> delete_compute_ca74d707a
    delete_compute_ca74d707a >> end_flow_task
