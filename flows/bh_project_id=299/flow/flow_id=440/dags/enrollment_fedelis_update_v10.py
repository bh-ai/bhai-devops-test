...
common_task = CommonTask(dag_id='enrollment_fedelis_update_v10', dag_params={})
lineage_task = LineageTask(dag_id='enrollment_fedelis_update_v10', dag_params={})
...
with DAG(
    dag_id='enrollment_fedelis_update_v10',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 440,
            'flow_name': 'enrollment_fedelis_update_v10',
            'flow_key': 'enrollment_fedelis_update_v10',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_2e8432c4b(**context):
        ...
        payload = (
            {
                "cluster_name": "fedelis_enrollment_v10",
    ...
    create_compute_2e8432c4b = PythonOperator(
        task_id='create_compute_2e8432c4b',
        python_callable=create_databricks_cluster_create_compute_2e8432c4b,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_2e8432c4b",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_ac142feda_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=685/enrollment_fedelis_update_v10_266.json",
                "databricks",
    ...
    submit_job_ac142feda = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_ac142feda',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_6eb4c91b1_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=686/silver_raw_fidelis_enrollment_tin_join_to_members_260511_878c.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_6eb4c91b1 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_6eb4c91b1',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_2e8432c4b", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_2e8432c4b or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_8c44aa466 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_8c44aa466',
    ...
    start_flow_task >> create_compute_2e8432c4b
    create_compute_2e8432c4b >> submit_job_ac142feda
    submit_job_ac142feda >> submit_job_6eb4c91b1
    create_compute_2e8432c4b >> submit_job_6eb4c91b1
    submit_job_6eb4c91b1 >> delete_compute_8c44aa466
    create_compute_2e8432c4b >> delete_compute_8c44aa466
    delete_compute_8c44aa466 >> end_flow_task
