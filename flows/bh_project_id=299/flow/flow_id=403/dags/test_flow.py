...
common_task = CommonTask(dag_id='test_flow', dag_params={})
lineage_task = LineageTask(dag_id='test_flow', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_5684adf61(**context):
    ...
    create_compute_5684adf61 = PythonOperator(
        task_id='create_compute_5684adf61',
        python_callable=create_databricks_cluster_create_compute_5684adf61,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_5684adf61", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_5684adf61 or params; skipping terminate")
            return
    ...
    _terminate_params = {}
    delete_compute_0cb45d929 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_0cb45d929',
    ...
    start_flow_task >> create_compute_5684adf61
    create_compute_5684adf61 >> delete_compute_0cb45d929
    delete_compute_0cb45d929 >> end_flow_task
