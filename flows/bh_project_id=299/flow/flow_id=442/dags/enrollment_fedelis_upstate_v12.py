...
common_task = CommonTask(dag_id='enrollment_fedelis_upstate_v12', dag_params={})
lineage_task = LineageTask(dag_id='enrollment_fedelis_upstate_v12', dag_params={})
...
with DAG(
    dag_id='enrollment_fedelis_upstate_v12',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 442,
            'flow_name': 'enrollment_fedelis_upstate_v12',
            'flow_key': 'enrollment_fedelis_upstate_v12',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_85b459d5f(**context):
        ...
        payload = (
            {
                "cluster_name": "enrollment_fedelis_upstate_v12",
    ...
    create_compute_85b459d5f = PythonOperator(
        task_id='create_compute_85b459d5f',
        python_callable=create_databricks_cluster_create_compute_85b459d5f,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_85b459d5f",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_ab243844d_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=691/enrollment_fedelis_upstate_v12_269.json",
                "databricks",
    ...
    submit_job_ab243844d = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_ab243844d',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_39d431ac0_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=692/silver_raw_fidelis_enrollment_tin_join_to_members_260511_8029.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_39d431ac0 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_39d431ac0',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_85b459d5f", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_85b459d5f or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_11844bbfb = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_11844bbfb',
    ...
    start_flow_task >> create_compute_85b459d5f
    create_compute_85b459d5f >> submit_job_ab243844d
    submit_job_ab243844d >> submit_job_39d431ac0
    create_compute_85b459d5f >> submit_job_39d431ac0
    submit_job_39d431ac0 >> delete_compute_11844bbfb
    create_compute_85b459d5f >> delete_compute_11844bbfb
    delete_compute_11844bbfb >> end_flow_task
