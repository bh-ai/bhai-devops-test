...
common_task = CommonTask(dag_id='cinq09', dag_params={})
lineage_task = LineageTask(dag_id='cinq09', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_94a4b3b16(**context):
    ...
    create_compute_94a4b3b16 = PythonOperator(
        task_id='create_compute_94a4b3b16',
        python_callable=create_databricks_cluster_create_compute_94a4b3b16,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_94a4b3b16",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_39841e7ed_{{ ts_nodash }}",
    ...
    submit_job_39841e7ed = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_39841e7ed',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_38e46538b_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=627/silver_raw_cinqcare_member_file_to_members_enrollment_segments_260508_cf9f.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_38e46538b = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_38e46538b',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_94a4b3b16", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_94a4b3b16 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_2fa472f2d = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_2fa472f2d',
    ...
    start_flow_task >> create_compute_94a4b3b16
    create_compute_94a4b3b16 >> submit_job_39841e7ed
    submit_job_39841e7ed >> submit_job_38e46538b
    create_compute_94a4b3b16 >> submit_job_38e46538b
    submit_job_38e46538b >> delete_compute_2fa472f2d
    create_compute_94a4b3b16 >> delete_compute_2fa472f2d
    delete_compute_2fa472f2d >> end_flow_task
