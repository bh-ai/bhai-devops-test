...
common_task = CommonTask(dag_id='enrollment_fedelis_upstate_v4', dag_params={})
lineage_task = LineageTask(dag_id='enrollment_fedelis_upstate_v4', dag_params={})
...
with DAG(
    dag_id='enrollment_fedelis_upstate_v4',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 437,
            'flow_name': 'enrollment_fedelis_upstate_v4',
            'flow_key': 'enrollment_fedelis_upstate_v4',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_6574714e0(**context):
        ...
        payload = (
            {
                "cluster_name": "fedelis_enrollment_v4",
    ...
    create_compute_6574714e0 = PythonOperator(
        task_id='create_compute_6574714e0',
        python_callable=create_databricks_cluster_create_compute_6574714e0,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_6574714e0",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_f8d49cdeb_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=671/enrollment_fedelis_upstate_v4_255.json",
                "databricks",
    ...
    submit_job_f8d49cdeb = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_f8d49cdeb',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_183410c77_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=672/silver_raw_fidelis_enrollment_tin_join_to_members_260511_8ede.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_183410c77 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_183410c77',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_6574714e0", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_6574714e0 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_ab3404de1 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_ab3404de1',
    ...
    start_flow_task >> create_compute_6574714e0
    create_compute_6574714e0 >> submit_job_f8d49cdeb
    submit_job_f8d49cdeb >> submit_job_183410c77
    create_compute_6574714e0 >> submit_job_183410c77
    submit_job_183410c77 >> delete_compute_ab3404de1
    create_compute_6574714e0 >> delete_compute_ab3404de1
    delete_compute_ab3404de1 >> end_flow_task
