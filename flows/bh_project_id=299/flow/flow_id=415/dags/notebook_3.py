...
common_task = CommonTask(dag_id='notebook_3', dag_params={})
lineage_task = LineageTask(dag_id='notebook_3', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_fc1465342(**context):
        ...
        payload = (
            {
                "cluster_name": "dbricks_test_cluster_02",
    ...
    create_compute_fc1465342 = PythonOperator(
        task_id='create_compute_fc1465342',
        python_callable=create_databricks_cluster_create_compute_fc1465342,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_fc1465342",
        "job_config": {
            "job_type": "notebook",
    ...
    submit_notebook_856430d5b = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_856430d5b',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_fc1465342", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_fc1465342 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_2c045e9ed = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_2c045e9ed',
    ...
    start_flow_task >> create_compute_fc1465342
    create_compute_fc1465342 >> submit_notebook_856430d5b
    submit_notebook_856430d5b >> delete_compute_2c045e9ed
    create_compute_fc1465342 >> delete_compute_2c045e9ed
    delete_compute_2c045e9ed >> end_flow_task
