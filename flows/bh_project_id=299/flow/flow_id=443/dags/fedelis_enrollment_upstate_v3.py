...
common_task = CommonTask(dag_id='fedelis_enrollment_upstate_v3', dag_params={})
lineage_task = LineageTask(dag_id='fedelis_enrollment_upstate_v3', dag_params={})
...
with DAG(
    dag_id='fedelis_enrollment_upstate_v3',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 443,
            'flow_name': 'fedelis_enrollment_upstate_v3',
            'flow_key': 'fedelis_enrollment_upstate_v3',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_4954fe22d(**context):
        ...
        payload = (
            {
                "cluster_name": "fedelis_enrollment_upstate_v3",
    ...
    create_compute_4954fe22d = PythonOperator(
        task_id='create_compute_4954fe22d',
        python_callable=create_databricks_cluster_create_compute_4954fe22d,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_4954fe22d",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_5064be3f5_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=693/fedelis_enrollment_update_v3_271.json",
                "databricks",
    ...
    submit_job_5064be3f5 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_5064be3f5',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_6824501a5_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=694/silver_raw_fidelis_enrollment_to_members_dedupe_load_260511_f2b7.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_6824501a5 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_6824501a5',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_4954fe22d", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_4954fe22d or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_fca454a75 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_fca454a75',
    ...
    start_flow_task >> create_compute_4954fe22d
    create_compute_4954fe22d >> submit_job_5064be3f5
    submit_job_5064be3f5 >> submit_job_6824501a5
    create_compute_4954fe22d >> submit_job_6824501a5
    submit_job_6824501a5 >> delete_compute_fca454a75
    create_compute_4954fe22d >> delete_compute_fca454a75
    delete_compute_fca454a75 >> end_flow_task
