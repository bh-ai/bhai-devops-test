...
common_task = CommonTask(dag_id='cinq08', dag_params={})
lineage_task = LineageTask(dag_id='cinq08', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_e2545610d(**context):
    ...
    create_compute_e2545610d = PythonOperator(
        task_id='create_compute_e2545610d',
        python_callable=create_databricks_cluster_create_compute_e2545610d,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_e2545610d",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_0b2400d5e_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=618/cinq08.json",
                "databricks",
    ...
    submit_job_0b2400d5e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_0b2400d5e',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_1f64094a6_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=619/silver_raw_cinqcare_member_file_to_members_enrollment_load_260508_ecee.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_1f64094a6 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_1f64094a6',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_e2545610d", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_e2545610d or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_5a941bc47 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_5a941bc47',
    ...
    start_flow_task >> create_compute_e2545610d
    create_compute_e2545610d >> submit_job_0b2400d5e
    submit_job_0b2400d5e >> submit_job_1f64094a6
    create_compute_e2545610d >> submit_job_1f64094a6
    submit_job_1f64094a6 >> delete_compute_5a941bc47
    create_compute_e2545610d >> delete_compute_5a941bc47
    delete_compute_5a941bc47 >> end_flow_task
