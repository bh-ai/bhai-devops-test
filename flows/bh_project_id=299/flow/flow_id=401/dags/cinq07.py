...
common_task = CommonTask(dag_id='cinq07', dag_params={})
lineage_task = LineageTask(dag_id='cinq07', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_4464aa669(**context):
    ...
    create_compute_4464aa669 = PythonOperator(
        task_id='create_compute_4464aa669',
        python_callable=create_databricks_cluster_create_compute_4464aa669,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_4464aa669",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_96c403d0f_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=616/cinq07.json",
                "databricks",
    ...
    submit_job_96c403d0f = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_96c403d0f',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_ef644097e_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=617/silver_raw_cinqcare_member_file_to_members_enrollment_load_260507_36c5.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_ef644097e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_ef644097e',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_4464aa669", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_4464aa669 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_3e540bfa0 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_3e540bfa0',
    ...
    start_flow_task >> create_compute_4464aa669
    create_compute_4464aa669 >> submit_job_96c403d0f
    submit_job_96c403d0f >> submit_job_ef644097e
    create_compute_4464aa669 >> submit_job_ef644097e
    submit_job_ef644097e >> delete_compute_3e540bfa0
    create_compute_4464aa669 >> delete_compute_3e540bfa0
    delete_compute_3e540bfa0 >> end_flow_task
