...
common_task = CommonTask(dag_id='fedelis_enrollment_upstate_v4', dag_params={})
lineage_task = LineageTask(dag_id='fedelis_enrollment_upstate_v4', dag_params={})
...
with DAG(
    dag_id='fedelis_enrollment_upstate_v4',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 444,
            'flow_name': 'fedelis_enrollment_upstate_v4',
            'flow_key': 'fedelis_enrollment_upstate_v4',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_6db48a192(**context):
        ...
        payload = (
            {
                "cluster_name": "fedelis_enrollment_upstate_v4",
    ...
    create_compute_6db48a192 = PythonOperator(
        task_id='create_compute_6db48a192',
        python_callable=create_databricks_cluster_create_compute_6db48a192,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_6db48a192",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_9f94a6cff_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=695/fedelis_enrollment_upstate_v4_272.json",
                "databricks",
    ...
    submit_job_9f94a6cff = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_9f94a6cff',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_9dc42a210_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=696/silver_raw_fidelis_enrollment_to_members_dedupe_load_260511_da38.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_9dc42a210 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_9dc42a210',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_6db48a192", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_6db48a192 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_27d459c27 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_27d459c27',
    ...
    start_flow_task >> create_compute_6db48a192
    create_compute_6db48a192 >> submit_job_9f94a6cff
    submit_job_9f94a6cff >> submit_job_9dc42a210
    create_compute_6db48a192 >> submit_job_9dc42a210
    submit_job_9dc42a210 >> delete_compute_27d459c27
    create_compute_6db48a192 >> delete_compute_27d459c27
    delete_compute_27d459c27 >> end_flow_task
