...
common_task = CommonTask(dag_id='fedelis_enrollment_v4', dag_params={})
lineage_task = LineageTask(dag_id='fedelis_enrollment_v4', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 432,
            'flow_name': 'fedelis_enrollment_v4',
            'flow_key': 'fedelis_enrollment_v4',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_6c04543a7(**context):
    ...
    create_compute_6c04543a7 = PythonOperator(
        task_id='create_compute_6c04543a7',
        python_callable=create_databricks_cluster_create_compute_6c04543a7,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_6c04543a7",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_da343028e_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=668/enrollment_fedelis_update_253.json",
                "databricks",
    ...
    submit_job_da343028e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_da343028e',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_ab54c9f1e_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=669/silver_raw_fidelis_enrollment_tin_join_to_members_260511_8451.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_ab54c9f1e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_ab54c9f1e',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_6c04543a7", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_6c04543a7 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_2df4b6d12 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_2df4b6d12',
    ...
    start_flow_task >> create_compute_6c04543a7
    create_compute_6c04543a7 >> submit_job_da343028e
    submit_job_da343028e >> submit_job_ab54c9f1e
    create_compute_6c04543a7 >> submit_job_ab54c9f1e
    submit_job_ab54c9f1e >> delete_compute_2df4b6d12
    create_compute_6c04543a7 >> delete_compute_2df4b6d12
    delete_compute_2df4b6d12 >> end_flow_task
