...
common_task = CommonTask(dag_id='cinq15', dag_params={})
lineage_task = LineageTask(dag_id='cinq15', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_4a94fa0f5(**context):
    ...
    create_compute_4a94fa0f5 = PythonOperator(
        task_id='create_compute_4a94fa0f5',
        python_callable=create_databricks_cluster_create_compute_4a94fa0f5,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_4a94fa0f5",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_3c8434051_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=644/cinq15.json",
                "databricks",
    ...
    submit_job_3c8434051 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_3c8434051',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_cfa46d0d2_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=645/silver_raw_centene_ga_risk_enrollment_to_members_tables_260508_99bb.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_cfa46d0d2 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_cfa46d0d2',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_4a94fa0f5", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_4a94fa0f5 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_f1340e1d4 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_f1340e1d4',
    ...
    start_flow_task >> create_compute_4a94fa0f5
    create_compute_4a94fa0f5 >> submit_job_3c8434051
    submit_job_3c8434051 >> submit_job_cfa46d0d2
    create_compute_4a94fa0f5 >> submit_job_cfa46d0d2
    submit_job_cfa46d0d2 >> delete_compute_f1340e1d4
    create_compute_4a94fa0f5 >> delete_compute_f1340e1d4
    delete_compute_f1340e1d4 >> end_flow_task
