...
common_task = CommonTask(dag_id='fedelis_enrollment_v2', dag_params={})
lineage_task = LineageTask(dag_id='fedelis_enrollment_v2', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 427,
            'flow_name': 'fedelis-enrollment-v2',
            'flow_key': 'fedelis_enrollment_v2',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_ddf4fc421(**context):
        ...
        payload = (
            {
                "cluster_name": "fedelis_enrollment_v2",
    ...
    create_compute_ddf4fc421 = PythonOperator(
        task_id='create_compute_ddf4fc421',
        python_callable=create_databricks_cluster_create_compute_ddf4fc421,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_ddf4fc421",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_94246d907_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=657/fedelis_enrollment_upstate_247.json",
                "databricks",
    ...
    submit_job_94246d907 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_94246d907',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_4fc4f6d9e_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=658/silver_raw_fidelis_enrollment_to_members_dedupe_load_260510_8866.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_4fc4f6d9e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_4fc4f6d9e',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_ddf4fc421", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_ddf4fc421 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_3444b055b = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_3444b055b',
    ...
    start_flow_task >> create_compute_ddf4fc421
    create_compute_ddf4fc421 >> submit_job_94246d907
    submit_job_94246d907 >> submit_job_4fc4f6d9e
    create_compute_ddf4fc421 >> submit_job_4fc4f6d9e
    submit_job_4fc4f6d9e >> delete_compute_3444b055b
    create_compute_ddf4fc421 >> delete_compute_3444b055b
    delete_compute_3444b055b >> end_flow_task
