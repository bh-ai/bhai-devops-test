...
common_task = CommonTask(dag_id='aks_flow_1', dag_params={})
lineage_task = LineageTask(dag_id='aks_flow_1', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_a104a01f5(**context):
    ...
    create_compute_a104a01f5 = PythonOperator(
        task_id='create_compute_a104a01f5',
        python_callable=create_databricks_cluster_create_compute_a104a01f5,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_a104a01f5",
        "job_config": {
    ...
    submit_job_7ba4bb1f5 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_7ba4bb1f5',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_a104a01f5", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_a104a01f5 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_e4c404457 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_e4c404457',
    ...
    start_flow_task >> create_compute_a104a01f5
    create_compute_a104a01f5 >> submit_job_7ba4bb1f5
    submit_job_7ba4bb1f5 >> delete_compute_e4c404457
    create_compute_a104a01f5 >> delete_compute_e4c404457
    delete_compute_e4c404457 >> end_flow_task
