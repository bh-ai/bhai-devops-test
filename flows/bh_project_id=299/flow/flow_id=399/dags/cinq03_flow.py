...
common_task = CommonTask(dag_id='cinq03_flow', dag_params={})
lineage_task = LineageTask(dag_id='cinq03_flow', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 399,
            'flow_name': 'cinq03-flow',
            'flow_key': 'cinq03_flow',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_5b4438e27(**context):
        ...
        payload = (
            {
                "cluster_name": "cinq03-pipeline-cluster",
    ...
    create_compute_5b4438e27 = PythonOperator(
        task_id='create_compute_5b4438e27',
        python_callable=create_databricks_cluster_create_compute_5b4438e27,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_5b4438e27",
        "job_config": {
    ...
    submit_job_a5645621a = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_a5645621a',
    ...
    submit_job_003476d55 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_003476d55',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_5b4438e27", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_5b4438e27 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_df24c10d6 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_df24c10d6',
    ...
    start_flow_task >> create_compute_5b4438e27
    create_compute_5b4438e27 >> submit_job_a5645621a
    submit_job_a5645621a >> submit_job_003476d55
    create_compute_5b4438e27 >> submit_job_003476d55
    submit_job_003476d55 >> delete_compute_df24c10d6
    create_compute_5b4438e27 >> delete_compute_df24c10d6
    delete_compute_df24c10d6 >> end_flow_task
