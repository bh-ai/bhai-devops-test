...
common_task = CommonTask(dag_id='cinq04', dag_params={})
lineage_task = LineageTask(dag_id='cinq04', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_17c467687(**context):
    ...
    create_compute_17c467687 = PythonOperator(
        task_id='create_compute_17c467687',
        python_callable=create_databricks_cluster_create_compute_17c467687,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_17c467687",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_4284362e3_{{ ts_nodash }}",
    ...
    submit_job_4284362e3 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_4284362e3',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_f3b4116b3_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
    ...
    submit_job_f3b4116b3 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_f3b4116b3',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_17c467687", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_17c467687 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_b4f4422d2 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_b4f4422d2',
    ...
    start_flow_task >> create_compute_17c467687
    create_compute_17c467687 >> submit_job_4284362e3
    submit_job_4284362e3 >> submit_job_f3b4116b3
    create_compute_17c467687 >> submit_job_f3b4116b3
    submit_job_f3b4116b3 >> delete_compute_b4f4422d2
    create_compute_17c467687 >> delete_compute_b4f4422d2
    delete_compute_b4f4422d2 >> end_flow_task
