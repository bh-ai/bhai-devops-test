...
common_task = CommonTask(dag_id='cinq14_withoutdq', dag_params={})
lineage_task = LineageTask(dag_id='cinq14_withoutdq', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 411,
            'flow_name': 'cinq14_withoutdq',
            'flow_key': 'cinq14_withoutdq',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_44a4cbc53(**context):
        ...
        payload = (
            {
                "cluster_name": "cinq14_withoutdq_cluster",
    ...
    create_compute_44a4cbc53 = PythonOperator(
        task_id='create_compute_44a4cbc53',
        python_callable=create_databricks_cluster_create_compute_44a4cbc53,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_44a4cbc53",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_0bb4c10e4_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=642/cinq14_without_dq.json",
                "databricks",
    ...
    submit_job_0bb4c10e4 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_0bb4c10e4',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_2044dfa25_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=643/silver_raw_centene_ga_risk_enrollment_to_members_tables_260508_ac9c.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_2044dfa25 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_2044dfa25',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_44a4cbc53", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_44a4cbc53 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_6794db530 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_6794db530',
    ...
    start_flow_task >> create_compute_44a4cbc53
    create_compute_44a4cbc53 >> submit_job_0bb4c10e4
    submit_job_0bb4c10e4 >> submit_job_2044dfa25
    create_compute_44a4cbc53 >> submit_job_2044dfa25
    submit_job_2044dfa25 >> delete_compute_6794db530
    create_compute_44a4cbc53 >> delete_compute_6794db530
    delete_compute_6794db530 >> end_flow_task
