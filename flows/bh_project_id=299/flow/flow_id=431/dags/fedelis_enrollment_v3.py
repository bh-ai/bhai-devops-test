...
common_task = CommonTask(dag_id='fedelis_enrollment_v3', dag_params={})
lineage_task = LineageTask(dag_id='fedelis_enrollment_v3', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 431,
            'flow_name': 'fedelis-enrollment-v3',
            'flow_key': 'fedelis_enrollment_v3',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_36747c08a(**context):
        ...
        payload = (
            {
                "cluster_name": "fedelis_enrollment_v3",
    ...
    create_compute_36747c08a = PythonOperator(
        task_id='create_compute_36747c08a',
        python_callable=create_databricks_cluster_create_compute_36747c08a,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_36747c08a",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_e2e4dd345_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=666/enrollment_fedelis_upstate_252.json",
                "databricks",
    ...
    submit_job_e2e4dd345 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_e2e4dd345',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_4174c5326_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=667/silver_raw_fidelis_enrollment_to_members_load_260511_d099.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_4174c5326 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_4174c5326',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_36747c08a", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_36747c08a or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_9c047c6ab = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_9c047c6ab',
    ...
    start_flow_task >> create_compute_36747c08a
    create_compute_36747c08a >> submit_job_e2e4dd345
    submit_job_e2e4dd345 >> submit_job_4174c5326
    create_compute_36747c08a >> submit_job_4174c5326
    submit_job_4174c5326 >> delete_compute_9c047c6ab
    create_compute_36747c08a >> delete_compute_9c047c6ab
    delete_compute_9c047c6ab >> end_flow_task
