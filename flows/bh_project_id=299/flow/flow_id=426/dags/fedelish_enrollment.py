...
common_task = CommonTask(dag_id='fedelish_enrollment', dag_params={})
lineage_task = LineageTask(dag_id='fedelish_enrollment', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 426,
            'flow_name': 'fedelish-enrollment',
            'flow_key': 'fedelish_enrollment',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_42e400316(**context):
        ...
        payload = (
            {
                "cluster_name": "fedelis_enrollment",
    ...
    create_compute_42e400316 = PythonOperator(
        task_id='create_compute_42e400316',
        python_callable=create_databricks_cluster_create_compute_42e400316,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_42e400316",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_7d84d5465_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=655/fedelis_upsate_enrollment_246.json",
                "databricks",
    ...
    submit_job_7d84d5465 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_7d84d5465',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_2b84afba0_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=656/silver_raw_fidelis_ny_upstate_to_members_dedupe_load_260510_ee27.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_2b84afba0 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_2b84afba0',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_42e400316", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_42e400316 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_f6c47a509 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_f6c47a509',
    ...
    start_flow_task >> create_compute_42e400316
    create_compute_42e400316 >> submit_job_7d84d5465
    submit_job_7d84d5465 >> submit_job_2b84afba0
    create_compute_42e400316 >> submit_job_2b84afba0
    submit_job_2b84afba0 >> delete_compute_f6c47a509
    create_compute_42e400316 >> delete_compute_f6c47a509
    delete_compute_f6c47a509 >> end_flow_task
