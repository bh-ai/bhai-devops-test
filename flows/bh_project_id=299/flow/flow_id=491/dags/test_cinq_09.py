...
common_task = CommonTask(dag_id='test_cinq_09', dag_params={})
lineage_task = LineageTask(dag_id='test_cinq_09', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 491,
            'flow_name': 'test-cinq-09',
            'flow_key': 'test_cinq_09',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_af540b11e(**context):
    ...
    _submit_params = {
        "compute_task_id": "create_compute_af540b11e",
        "job_config": {
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_af540b11e", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_af540b11e or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_f894bd8b4 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_f894bd8b4',
    ...
    start_flow_task >> create_compute_af540b11e
    create_compute_af540b11e >> submit_job_52a44ce01
    submit_job_52a44ce01 >> delete_compute_f894bd8b4
    create_compute_af540b11e >> delete_compute_f894bd8b4
    delete_compute_f894bd8b4 >> end_flow_task
