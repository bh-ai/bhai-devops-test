...
common_task = CommonTask(dag_id='notebook_5', dag_params={})
lineage_task = LineageTask(dag_id='notebook_5', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_0c94a9f8a(**context):
    ...
    create_compute_0c94a9f8a = PythonOperator(
        task_id='create_compute_0c94a9f8a',
        python_callable=create_databricks_cluster_create_compute_0c94a9f8a,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_0c94a9f8a",
        "job_config": {
            "job_type": "notebook",
    ...
    submit_notebook_d91490937 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_d91490937',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_0c94a9f8a", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_0c94a9f8a or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_51b413ec1 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_51b413ec1',
    ...
    start_flow_task >> create_compute_0c94a9f8a
    create_compute_0c94a9f8a >> submit_notebook_d91490937
    submit_notebook_d91490937 >> delete_compute_51b413ec1
    create_compute_0c94a9f8a >> delete_compute_51b413ec1
    delete_compute_51b413ec1 >> end_flow_task
