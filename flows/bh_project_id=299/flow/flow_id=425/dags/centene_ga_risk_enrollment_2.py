...
common_task = CommonTask(dag_id='centene_ga_risk_enrollment_2', dag_params={})
lineage_task = LineageTask(dag_id='centene_ga_risk_enrollment_2', dag_params={})
...
with DAG(
    dag_id='centene_ga_risk_enrollment_2',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 425,
            'flow_name': 'centene_ga_risk_enrollment_2',
            'flow_key': 'centene_ga_risk_enrollment_2',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_ba34d42ae(**context):
        ...
        payload = (
            {
                "cluster_name": "centene_enrollment",
    ...
    create_compute_ba34d42ae = PythonOperator(
        task_id='create_compute_ba34d42ae',
        python_callable=create_databricks_cluster_create_compute_ba34d42ae,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_ba34d42ae",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_4ab4f6749_{{ ts_nodash }}",
    ...
    submit_job_4ab4f6749 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_4ab4f6749',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_8af43c72d_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
    ...
    submit_job_8af43c72d = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_8af43c72d',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_ba34d42ae", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_ba34d42ae or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_f5b471aca = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_f5b471aca',
    ...
    start_flow_task >> create_compute_ba34d42ae
    create_compute_ba34d42ae >> submit_job_4ab4f6749
    submit_job_4ab4f6749 >> submit_job_8af43c72d
    create_compute_ba34d42ae >> submit_job_8af43c72d
    submit_job_8af43c72d >> delete_compute_f5b471aca
    create_compute_ba34d42ae >> delete_compute_f5b471aca
    delete_compute_f5b471aca >> end_flow_task
