...
common_task = CommonTask(dag_id='enrollment_fedelis_update_v9', dag_params={})
lineage_task = LineageTask(dag_id='enrollment_fedelis_update_v9', dag_params={})
...
with DAG(
    dag_id='enrollment_fedelis_update_v9',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 439,
            'flow_name': 'enrollment_fedelis_update_v9',
            'flow_key': 'enrollment_fedelis_update_v9',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_6fa415eea(**context):
        ...
        payload = (
            {
                "cluster_name": "fedelis_enrollment_v9",
    ...
    create_compute_6fa415eea = PythonOperator(
        task_id='create_compute_6fa415eea',
        python_callable=create_databricks_cluster_create_compute_6fa415eea,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_6fa415eea",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_c114ecfd2_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=681/enrollment_fedelis_upstate_v9_262.json",
                "databricks",
    ...
    submit_job_c114ecfd2 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_c114ecfd2',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_14048cd19_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=682/silver_raw_fidelis_enrollment_dedupe_join_to_members_260511_7951.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_14048cd19 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_14048cd19',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_6fa415eea", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_6fa415eea or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_1c44b223f = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_1c44b223f',
    ...
    start_flow_task >> create_compute_6fa415eea
    create_compute_6fa415eea >> submit_job_c114ecfd2
    submit_job_c114ecfd2 >> submit_job_14048cd19
    create_compute_6fa415eea >> submit_job_14048cd19
    submit_job_14048cd19 >> delete_compute_1c44b223f
    create_compute_6fa415eea >> delete_compute_1c44b223f
    delete_compute_1c44b223f >> end_flow_task
