...
common_task = CommonTask(dag_id='cinq12', dag_params={})
lineage_task = LineageTask(dag_id='cinq12', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_76a4f2cca(**context):
    ...
    create_compute_76a4f2cca = PythonOperator(
        task_id='create_compute_76a4f2cca',
        python_callable=create_databricks_cluster_create_compute_76a4f2cca,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_76a4f2cca",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_1a64fbb80_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=636/cinq12.json",
                "databricks",
    ...
    submit_job_1a64fbb80 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_1a64fbb80',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_28a4a584d_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=637/silver_raw_centene_ga_risk_enrollment_to_members_load_260508_ba11.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_28a4a584d = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_28a4a584d',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_76a4f2cca", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_76a4f2cca or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_396449164 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_396449164',
    ...
    start_flow_task >> create_compute_76a4f2cca
    create_compute_76a4f2cca >> submit_job_1a64fbb80
    submit_job_1a64fbb80 >> submit_job_28a4a584d
    create_compute_76a4f2cca >> submit_job_28a4a584d
    submit_job_28a4a584d >> delete_compute_396449164
    create_compute_76a4f2cca >> delete_compute_396449164
    delete_compute_396449164 >> end_flow_task
