...
common_task = CommonTask(dag_id='notebook_test', dag_params={})
lineage_task = LineageTask(dag_id='notebook_test', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 414,
            'flow_name': 'notebook-test',
            'flow_key': 'notebook_test',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_d734ebadb(**context):
    ...
    create_compute_d734ebadb = PythonOperator(
        task_id='create_compute_d734ebadb',
        python_callable=create_databricks_cluster_create_compute_d734ebadb,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_notebook_params = {
        "compute_task_id": "create_compute_d734ebadb",
        "job_config": {
            "job_type": "notebook",
    ...
    submit_notebook_d7f491468 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_d7f491468',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_d734ebadb", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_d734ebadb or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
    ...
    _terminate_params = {}
    delete_compute_14b44127a = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_14b44127a',
    ...
    start_flow_task >> create_compute_d734ebadb
    create_compute_d734ebadb >> submit_notebook_d7f491468
    submit_notebook_d7f491468 >> delete_compute_14b44127a
    create_compute_d734ebadb >> delete_compute_14b44127a
    delete_compute_14b44127a >> end_flow_task
