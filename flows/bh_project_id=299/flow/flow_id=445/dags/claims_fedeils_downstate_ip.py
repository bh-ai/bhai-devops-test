...
common_task = CommonTask(dag_id='claims_fedeils_downstate_ip', dag_params={})
lineage_task = LineageTask(dag_id='claims_fedeils_downstate_ip', dag_params={})
...
with DAG(
    dag_id='claims_fedeils_downstate_ip',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 445,
            'flow_name': 'claims_fedeils_downstate_ip',
            'flow_key': 'claims_fedeils_downstate_ip',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_b4b453a79(**context):
        ...
        payload = (
            {
                "cluster_name": "claims_fedeils_downstate_ip",
    ...
    create_compute_b4b453a79 = PythonOperator(
        task_id='create_compute_b4b453a79',
        python_callable=create_databricks_cluster_create_compute_b4b453a79,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_b4b453a79",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_e07420bdf_{{ ts_nodash }}",
    ...
    submit_job_e07420bdf = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_e07420bdf',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_79548a1b5_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
    ...
    submit_job_79548a1b5 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_79548a1b5',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_b4b453a79", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_b4b453a79 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_6604411ac = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_6604411ac',
    ...
    start_flow_task >> create_compute_b4b453a79
    create_compute_b4b453a79 >> submit_job_e07420bdf
    submit_job_e07420bdf >> submit_job_79548a1b5
    create_compute_b4b453a79 >> submit_job_79548a1b5
    submit_job_79548a1b5 >> delete_compute_6604411ac
    create_compute_b4b453a79 >> delete_compute_6604411ac
    delete_compute_6604411ac >> end_flow_task
