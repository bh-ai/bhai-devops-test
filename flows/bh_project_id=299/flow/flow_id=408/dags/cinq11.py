...
common_task = CommonTask(dag_id='cinq11', dag_params={})
lineage_task = LineageTask(dag_id='cinq11', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_aee4157fc(**context):
    ...
    create_compute_aee4157fc = PythonOperator(
        task_id='create_compute_aee4157fc',
        python_callable=create_databricks_cluster_create_compute_aee4157fc,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_aee4157fc",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_404411a26_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=632/cinq11.json",
                "databricks",
    ...
    submit_job_404411a26 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_404411a26',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_94848122a_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=633/silver_raw_centene_ga_enrollment_to_members_pipeline_260508_f59c.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_94848122a = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_94848122a',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_aee4157fc", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_aee4157fc or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_07e4efc6f = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_07e4efc6f',
    ...
    start_flow_task >> create_compute_aee4157fc
    create_compute_aee4157fc >> submit_job_404411a26
    submit_job_404411a26 >> submit_job_94848122a
    create_compute_aee4157fc >> submit_job_94848122a
    submit_job_94848122a >> delete_compute_07e4efc6f
    create_compute_aee4157fc >> delete_compute_07e4efc6f
    delete_compute_07e4efc6f >> end_flow_task
