...
common_task = CommonTask(dag_id='enrollment_fidelis_downstate_v3_492_561', dag_params={})
lineage_task = LineageTask(dag_id='enrollment_fidelis_downstate_v3_492_561', dag_params={})
...
with DAG(
    dag_id='enrollment_fidelis_downstate_v3_492_561',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 561,
            'flow_name': 'Enrollment_fidelis_downstate_v3_492',
            'flow_key': 'enrollment_fidelis_downstate_v3_492',
            'bh_project_id': 299,
    ...
    def validate_inbound_files(**context):
        ...
        warehouse_id = (
            params.get("databricks_warehouse_id")
            or Variable.get("default_databricks_warehouse_id", default_var=None)
        )
        control_catalog = (
                ...
                if batch_id is not None:
                    ...
                    if valid_files:
                        ...
                        if dup_keys:
                            ...
                            raise RuntimeError(
                                "Duplicate accepted input(s) already processed for this feed/source."
                            )
    ...
    _validate_params = {
        ...
        "source_prefix": "bhargs/1-Enrollment/1-Enrollment/Fedelis NY",
        ...
        "sources": [
            {
                ...
                "prefix": "bhargs/1-Enrollment/1-Enrollment/Fedelis NY",
                "filename_regex": "^safeharbor_CINQUPSTATE_Member_Roster_[0-9]{2}_[0-9]{2}_[0-9]{4}[.]csv$",
    ...
    _submit_params = {
        ...
        "job_config": {
            ...
            "name": "{{ dag.dag_id }}_run_pipelines_enrollment_fidelis_downstate_v3_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=965/enrollment_fidelis_downstate_v3.json",
                "databricks",
    ...
    run_pipelines_enrollment_fidelis_downstate_v3 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='run_pipelines_enrollment_fidelis_downstate_v3',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_run_pipelines_silver_raw_roster_dedupe_to_member_enrollment_load_260602_f81e_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=966/silver_raw_roster_dedupe_to_member_enrollment_load_260602_f81e.json",
                "databricks",
                "/Workspace/Shared/dev-utils/schemas"
            ]
        },
    ...
    run_pipelines_silver_raw_roster_dedupe_to_member_enrollment_load_260602_f81e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='run_pipelines_silver_raw_roster_dedupe_to_member_enrollment_load_260602_f81e',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    _archive_params = {
        "bucket_name": "my-test-bucket",
        "source_prefix": "",
        "archive_prefix": "Archive/bhargs/1-Enrollment/1-Enrollment/Fedelis NY",
        "cloud_type": "databricks",
        "airflow_connection_id": "databricks_default",
        "secret_name": "bh-dev-westus3-kv-key-scope/bh-azureblob-azureblob",
        "delete_source": True,
    ...
    create_compute >> run_pipelines_enrollment_fidelis_downstate_v3
    run_pipelines_enrollment_fidelis_downstate_v3 >> run_pipelines_silver_raw_roster_dedupe_to_member_enrollment_load_260602_f81e
    create_compute >> run_pipelines_silver_raw_roster_dedupe_to_member_enrollment_load_260602_f81e
    run_pipelines_silver_raw_roster_dedupe_to_member_enrollment_load_260602_f81e >> archive_processed_files
