...
common_task = CommonTask(dag_id='enrollment_fedelis_v2', dag_params={})
lineage_task = LineageTask(dag_id='enrollment_fedelis_v2', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 434,
            'flow_name': 'enrollment_fedelis_v2',
            'flow_key': 'enrollment_fedelis_v2',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_ac14098f2(**context):
        ...
        payload = (
            {
                "cluster_name": "enrollment_fedelis",
    ...
    create_compute_ac14098f2 = PythonOperator(
        task_id='create_compute_ac14098f2',
        python_callable=create_databricks_cluster_create_compute_ac14098f2,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_ac14098f2",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_a06410feb_{{ ts_nodash }}",
    ...
    submit_job_a06410feb = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_a06410feb',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_6ed4ed04d_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
    ...
    submit_job_6ed4ed04d = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_6ed4ed04d',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_ac14098f2", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_ac14098f2 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_97b476e15 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_97b476e15',
    ...
    start_flow_task >> create_compute_ac14098f2
    create_compute_ac14098f2 >> submit_job_a06410feb
    submit_job_a06410feb >> submit_job_6ed4ed04d
    create_compute_ac14098f2 >> submit_job_6ed4ed04d
    submit_job_6ed4ed04d >> delete_compute_97b476e15
    create_compute_ac14098f2 >> delete_compute_97b476e15
    delete_compute_97b476e15 >> end_flow_task
