...
common_task = CommonTask(dag_id='flow_fedelis_1', dag_params={})
lineage_task = LineageTask(dag_id='flow_fedelis_1', dag_params={})
...
with DAG(
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 472,
            'flow_name': 'flow-fedelis-1',
            'flow_key': 'flow_fedelis_1',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_2e64dab61(**context):
    ...
    create_compute_2e64dab61 = PythonOperator(
        task_id='create_compute_2e64dab61',
        python_callable=create_databricks_cluster_create_compute_2e64dab61,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_2e64dab61",
        "job_config": {
    ...
    submit_job_6b2412b0c = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_6b2412b0c',
    ...
    submit_job_74043cbfe = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_74043cbfe',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_2e64dab61", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_2e64dab61 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_08e45088b = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_08e45088b',
    ...
    start_flow_task >> create_compute_2e64dab61
    create_compute_2e64dab61 >> submit_job_6b2412b0c
    submit_job_6b2412b0c >> submit_job_74043cbfe
    create_compute_2e64dab61 >> submit_job_74043cbfe
    submit_job_74043cbfe >> delete_compute_08e45088b
    create_compute_2e64dab61 >> delete_compute_08e45088b
    delete_compute_08e45088b >> end_flow_task
