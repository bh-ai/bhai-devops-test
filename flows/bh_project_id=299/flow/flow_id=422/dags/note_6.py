...
common_task = CommonTask(dag_id='note_6', dag_params={})
lineage_task = LineageTask(dag_id='note_6', dag_params={})
...
with DAG(
    ...
    def create_databricks_cluster_create_compute_d8e4cb1ae(**context):
    ...
    create_compute_d8e4cb1ae = PythonOperator(
        task_id='create_compute_d8e4cb1ae',
        python_callable=create_databricks_cluster_create_compute_d8e4cb1ae,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_d8e4cb1ae",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_6994694e9_{{ ts_nodash }}",
    ...
    submit_job_6994694e9 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_6994694e9',
    ...
    _submit_notebook_params = {
        ...
        "job_config": {
            "job_type": "notebook",
            "notebook_path": "/Workspace/Users/sathish@bighammer.ai/hello_world",
            "parameters": {}
        },
        "airflow_connection_id": "databricks_default",
        "compute_xcom_key": "return_value"
    }
    submit_notebook_404452a1e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_notebook_404452a1e',
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_d8e4cb1ae", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_d8e4cb1ae or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_d2a42c87e = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_d2a42c87e',
    ...
    start_flow_task >> create_compute_d8e4cb1ae
    create_compute_d8e4cb1ae >> submit_job_6994694e9
    submit_job_6994694e9 >> submit_notebook_404452a1e
    create_compute_d8e4cb1ae >> submit_notebook_404452a1e
    submit_notebook_404452a1e >> delete_compute_d2a42c87e
    create_compute_d8e4cb1ae >> delete_compute_d2a42c87e
    delete_compute_d2a42c87e >> end_flow_task
