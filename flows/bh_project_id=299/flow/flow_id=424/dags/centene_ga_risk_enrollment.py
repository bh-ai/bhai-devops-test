...
common_task = CommonTask(dag_id='centene_ga_risk_enrollment', dag_params={})
lineage_task = LineageTask(dag_id='centene_ga_risk_enrollment', dag_params={})
...
with DAG(
    dag_id='centene_ga_risk_enrollment',
    ...
    start_flow_task = PythonOperator(
        ...
        params = {
            'flow_id': 424,
            'flow_name': 'centene_ga_risk_enrollment',
            'flow_key': 'centene_ga_risk_enrollment',
            'bh_project_id': 299,
    ...
    def create_databricks_cluster_create_compute_3a64f5550(**context):
        from airflow_plugins.cloud_factory import CloudFactory
        hook = DatabricksHook(databricks_conn_id='databricks_defaultcen')
        conn = hook.get_conn()
        ...
        payload = (
            {
                "cluster_name": "centene_enrolment",
    ...
    create_compute_3a64f5550 = PythonOperator(
        task_id='create_compute_3a64f5550',
        python_callable=create_databricks_cluster_create_compute_3a64f5550,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    )
    ...
    _submit_params = {
        "compute_task_id": "create_compute_3a64f5550",
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_3da4fa71f_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=653/enrollment_centene_ga_risk_243.json",
                "databricks",
    ...
    submit_job_3da4fa71f = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_3da4fa71f',
    ...
    _submit_params = {
        ...
        "job_config": {
            "job_type": "spark_python",
            "name": "{{ dag.dag_id }}_submit_job_ee64922a9_{{ ts_nodash }}",
            "python_file": "/Workspace/Shared/bh-dev-utils/pipelines/main.py",
            "parameters": [
                "/Workspace/Shared/codespace/test/pipelines/bh_project_id=299/pipeline/pipeline_id=654/silver_raw_centene_ga_risk_enrollment_mapping_260510_dbfa.json",
                "databricks",
                "/Workspace/Shared/bh-dev-utils/schemas"
            ]
        },
    ...
    submit_job_ee64922a9 = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='submit_job_ee64922a9',
        python_callable=submit_job_to_cluster,
        params=_submit_params,
        on_success_callback=common_task.success_callback,
        on_failure_callback=common_task.failure_callback,
    ...
    def terminate_databricks_resources(**context):
        ti = context["ti"]
        compute_id = ti.xcom_pull(task_ids="create_compute_3a64f5550", key="return_value")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            params = context.get("params") or {}
            compute_id = params.get("compute_id")
        if not compute_id or (isinstance(compute_id, str) and "{" in compute_id):
            logger.warning("No compute_id from XCom task create_compute_3a64f5550 or params; skipping terminate")
            return
        from airflow.hooks.base import BaseHook
        conn = BaseHook.get_connection('databricks_default')
        workspace_url = (conn.host or '').rstrip('/')
    ...
    _terminate_params = {}
    delete_compute_eb54bf87b = PythonOperator(
        pre_execute=common_task.pre_execute_callback,
        task_id='delete_compute_eb54bf87b',
    ...
    start_flow_task >> create_compute_3a64f5550
    create_compute_3a64f5550 >> submit_job_3da4fa71f
    submit_job_3da4fa71f >> submit_job_ee64922a9
    create_compute_3a64f5550 >> submit_job_ee64922a9
    submit_job_ee64922a9 >> delete_compute_eb54bf87b
    create_compute_3a64f5550 >> delete_compute_eb54bf87b
    delete_compute_eb54bf87b >> end_flow_task
